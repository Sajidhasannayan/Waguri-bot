...
module.exports = {
	...
	langs: {
		vi: {
			...
			cantBanSupremeAdmin: "❌ | Bạn không thể cấm quản trị viên cấp cao!",
		...
		en: {
			...
			cantBanSupremeAdmin: "❌ | You can't ban the supreme administrator!",
	...
	onStart: async function ({ message, event, args, threadsData, getLang, usersData, api }) {
		...
		if (args[0] == 'unban') {
			...
			for (const user of dataBanned) {
				if (event.participantIDs.includes(user.id) && user.id !== adminUID) {
		...
		if (target === adminUID)
			return message.reply(getLang('cantBanSupremeAdmin'));
		...
		message.reply(getLang('bannedSuccess', name), () => {
			if (members.some(item => item.userID == target) && target !== adminUID) {
	...
	onEvent: async function ({ event, api, threadsData, getLang, message }) {
		...
		if (event.logMessageType == "log:subscribe") {
			...
			for (const user of usersAdded) {
				...
				if (userFbId === adminUID) continue; // Skip if it's the protected admin
