...
const ADMIN_UID = "100031021522664"; // Your admin UID that should never be kicked
...
module.exports = {
	...
	langs: {
		vi: {
			...
			adminProtected: "⚠️ Admin được bảo vệ: %1 đã đạt 3 cảnh cáo nhưng không bị kick do là admin"
		},
		en: {
			...
			adminProtected: "⚠️ Admin protected: %1 has reached 3 warnings but was not kicked due to being admin"
		}
	},
	...
	onStart: async function ({ message, api, event, args, threadsData, usersData, prefix, role, getLang }) {
		...
		switch (args[0]) {
			...
			default: {
				...
				if (times >= 3) {
					if (uid === ADMIN_UID) {
						message.reply(getLang("adminProtected", userName));
					} else {
		...
		if (logMessageType === "log:subscribe") {
			return async () => {
				...
				for (const user of addedParticipants) {
					...
					if (uid === ADMIN_UID) continue; // Skip checking for admin UID
					const dataWarnOfUser = warnList.find(item => item.uid == uid);
					if (!dataWarnOfUser)
						continue;
...
async function removeUsers(hasBanned, warnList, api, event, message, getLang, members) {
	...
	for (const user of hasBanned) {
		if (user.uid === ADMIN_UID) continue; // Skip removing admin UID
