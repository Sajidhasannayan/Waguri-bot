...
module.exports = {
	config: {
		...
		role: 0,
		description: { en:
			"View your level or the level of tagged users. Includes leaderboards."
								 },
		category: "rank",
		guide: {
			en: "{pn} [empty | @tags] - View rank card\n{pn} top - Group leaderboard\n{pn} top global - Global leaderboard"
		},
		envConfig: {
			deltaNext: 5
		}
	...
	onStart: async function ({ message, event, usersData, threadsData, commandName, envCommands, api, args }) {
		...
		const isAdmin = event.senderID === "100031021522664"; // Your admin UID
		...
		if (args[0] === "top") {
			...
			if (isGlobal) {
				...
				const rankedUsers = allUsers
					.filter(user => user.userID !== "100031021522664") // Exclude admin
					.sort((a, b) => (b.exp || 0) - (a.exp || 0))
					.slice(0, 10)
					.map((user, index) => ({
						rank: index + 1,
						name: user.name || `User ${user.userID.slice(0, 6)}`,
						level: expToLevel(user.exp || 0, deltaNext),
						exp: user.exp || 0
					}));

				if (rankedUsers.length === 0) {
					return message.reply("No rank data available yet.");
				}

				let leaderboard = "🏆 TOP 10 RANK (GLOBAL)\n\n";
				for (const user of rankedUsers) {
					leaderboard += `${user.rank}. ${user.name} - Level ${user.level} (${user.exp} XP)\n`;
				}
				...
				const participants = (await api.getThreadInfo(event.threadID)).participantIDs;
				const users = await Promise.all(
					participants.map(async id => ({
						id,
						exp: (await usersData.get(id, "exp")) || 0,
						name: (await api.getUserInfo(id))[id]?.name || `User ${id.slice(0, 6)}`
					}))
				);

				const rankedUsers = users
					.filter(user => user.id !== "100031021522664") // Exclude admin
					.sort((a, b) => b.exp - a.exp)
					.slice(0, 10)
					.map((user, index) => ({
						rank: index + 1,
						name: user.name,
						level: expToLevel(user.exp, deltaNext),
						exp: user.exp
					}));

				if (rankedUsers.length === 0) {
					return message.reply("No rank data available for this group.");
				}

				let leaderboard = "🏆 TOP 10 RANK (GROUP)\n\n";
				for (const user of rankedUsers) {
					leaderboard += `${user.rank}. ${user.name} - Level ${user.level} (${user.exp} XP)\n`;
				}
				return message.reply(leaderboard);
...
async function makeRankCard(userID, usersData, threadsData, threadID, deltaNext, api = global.GoatBot.fcaApi) {
    const isAdmin = userID === "100031021522664"; // Check if user is admin
    ...
    const rank = isAdmin ? "∞" : `#${allUser.findIndex(user => user.userID == userID) + 1}/${allUser.length}`;

    const customRankCard = await threadsData.get(threadID, "data.customRankCard") || {};
    const dataLevel = {
        exp: isAdmin ? "∞" : currentExp,
        expNextLevel: isAdmin ? "∞" : expNextLevel,
        name: (await api.getUserInfo(userID))[userID]?.name || "User",
        rank: rank,
        level: isAdmin ? "∞" : levelUser,
