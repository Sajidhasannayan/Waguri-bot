...
module.exports = {
  ...
  onStart: async function ({ event, message, args, api }) {
    ...
    if (prompt.toLowerCase() === "reset") {
      ...
      return message.reply("Goku session has been reset.");
    }

    api.setMessageReaction("⏳", event.messageID, () => {}, true);

    const response = await this.getGokuResponse(prompt, userId);
    if (!response) {
      api.setMessageReaction("❌", event.messageID, () => {}, true);
      return message.reply("Goku didn’t respond.");
    }
  ...
  getGokuResponse: async function (prompt, uid) {
    try {
      const url = `https://kaiz-apis.gleeze.com/api/goku?ask=${encodeURIComponent(prompt)}&uid=${uid}&apikey=828c7c9b-3017-4647-b757-22853d6c9dd5`;
      const { data } = await axios.get(url, { timeout: 15000 });

      return typeof data === "string" ? data : data.response || JSON.stringify(data);
    } catch (err) {
      console.error("Goku error:", err?.response?.data || err);
      return null;
    }
  }
};