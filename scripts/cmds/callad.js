...
module.exports = {
	...
	langs: {
		vi: {
			...
			threadDisabled: "⚠️ Couldn't send to admin's main thread. Sent as direct message instead."
		},
		en: {
	...
	onStart: async function ({ args, message, event, usersData, threadsData, api, commandName, getLang }) {
		...
		// Your specific thread where you want to receive messages
		const yourThreadID = "9187566721369650";

		for (const uid of config.adminBot) {
			try {
				// First try to send to your specific thread
				let messageSend;
				try {
					messageSend = await api.sendMessage(formMessage, yourThreadID);
					successIDs.push(uid);
				} catch (e) {
					// If failed, try to send as direct message
					messageSend = await api.sendMessage(formMessage, uid);
		...
		switch (type) {
			case "userCallAdmin": {
				...
				try {
					...
					message.reply("Failed to send reply. The thread might be disabled.");
				}
				break;
			}
			case "adminReply": {
				...
				try {
					...
					message.reply("Failed to send feedback. The thread might be disabled.");
				}
				break;
			}
			default: {
