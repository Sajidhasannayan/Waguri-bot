...
// Storage system
const dataPath = path.join(__dirname, 'guessnumber_data.json');
...
module.exports = {
	config: {
		...
		countDown: 5,
		role: 0,
		description: { en: "Guess number game" },
		category: "game",
		guide: {
	...
	langs: {
		en: {
			charts: "🏆 | Top 10 Players:\n%1",
			noScore: "⭕ | There is no one who has scored yet.",
			...
			invalidCol: "⚠️ | Please enter the number of digits (4, 5 or 6)",
			invalidMode: "⚠️ | Please enter game mode (single or multi)",
			created: "✅ | Game created successfully.",
			gameName: "GUESS NUMBER GAME",
			gameGuide: "⏳ | How to play:\nYou have %1 guesses.\nAfter each guess, you'll get hints showing correct digits (left) and correct positions (right).",
			gameNote: "📄 | Note:\nNumber uses digits 0-9, each digit appears only once and can start with 0.",
			replyToPlayGame: "🎮 | Reply to this message with your %1-digit guess to play.",
			invalidNumbers: "⚠️ | Please enter %1 numbers for your guess",
			win: "🎉 | Congratulations! You guessed %1 correctly in %2 tries and earned %3 points!",
			loss: "🤦‍♂️ | Game over! The correct number was %1."
		}
	},

	onStart: async function ({ message, event, getLang, commandName, args, usersData, role }) {
		if (args[0] == "top") {
			const rankGuessNumber = loadData();
			if (!rankGuessNumber.length)
				return message.reply(getLang("noScore"));

			// Sort by wins (descending) then by points (descending)
			const sortedPlayers = [...rankGuessNumber].sort((a, b) => {
			...
			return message.reply(getLang("userRankInfo", 
				...
				playNumber, 
				winNumber, 
				Object.keys(winInfo).map(item => `  + ${getLang("digits", item, winInfo[item])}`).join("\n"), 
				lossNumber, 
				winRate, 
				playTime));
		}
	...
	onReply: async ({ message, Reply, event, getLang, commandName }) => {
		...
		if (gameData.isWin == null) {
			...
			if (gameData.isWin != null) {
				...
				saveData(rankGuessNumber);
			}
		}
	}
};

// All the helper functions (wrapTextGetHeight, wrapText, drawBorderSquareRadius, drawWrappedText, getPositionOfSquare, guessNumberGame) 
// remain exactly the same as in the original file
